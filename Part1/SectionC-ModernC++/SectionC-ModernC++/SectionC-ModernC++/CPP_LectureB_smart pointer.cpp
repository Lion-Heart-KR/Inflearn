...
/// -- shared ptr은 특정 객체를 몇 개의 포인터가 참조하고 있는지를 카운팅 한다.
/// --- 이 경우 하나라도 특정 객체를 참조하고 있다면, 객체를 소멸시킬 수 없다.
/// --- 하지만, shared ptr을 사용하면 순환참조라는 문제가 생긴다.
/// --- k1이 k2를 가리키고, k2가 k1을 가리키고 있다면, 서로를 가리키는 포인터가 하나씩 있으므로, 객체가 영영 소멸될 수 없다.
/// --- 이 문제를 해결한 것은 weak ptr이다.
/// 
/// -- weak ptr은 순환참조가 발생할 가능성이 있는 포인터에 사용해주는 것 같다.
/// --- 그렇게 해서 weak ptr으로 선언한 변수가 참조하는 객체는... count가 0이 아니더라도 객체를 소멸시킬 수 있긴 한데
/// --- 대신에, deadpointer를 참조하는일을 막아야 하기 때문에 weak_ptr.expired()를 이용해서 객체가 아직 유효한지 확인을 하고
/// --- 유효하다면, weak_ptr.lock()을 이용해서 shared_ptr을 가져온다.
/// --- 그니까... weak ptr은 내가 지켜는 봐줄게! 하는 느낌이다.
/// --- shared ptr은 모두 내 통제 아래에 두겠어! 하는 느낌이고...
/// 
/// --- 사실 아직 잘 모르겠다. 
/// </summary>
/// <returns></returns>
